...
function showRiddle() {
  ...
  newQues.textContent = question
  const newAns = document.createElement('p')
  newAns.textContent = 'The answer is: '+answer
  newAns.setAttribute('id','ridAs')
  newAns.hidden = true
  const container = document.querySelector('#riddle')
  clearAll()
  container.appendChild(newQues)
...
function revealAnswers() {
  const container = document.querySelector('#riddle')
  const rid = container.querySelector('p')
  const newAns = document.querySelector('#ridAs')
  if (rid && newAns.hidden){
    newAns.hidden = false
  }else if(rid){
